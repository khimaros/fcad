...
- R4.4 the 3d geometry diff of the working tree vs git HEAD (green added / red
  ...
  because the booleans are the slow part it is split: `diff-build [TARGET]` bakes
  the result to `dist/<target>.diff.FCStd` headless, `diff-open [TARGET]` opens
  that instantly in the gui, and `diff [TARGET]` chains the two. it must not
  boolean signature-matched (unchanged) instances.
